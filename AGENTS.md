...
> **Gotcha — Godot export templates may be absent on a fresh snapshot boot.** The editor binary lives in the snapshot, but the ~1.2 GB export templates (`$XDG_DATA_HOME/godot/export_templates/4.7.stable/`, where `XDG_DATA_HOME=/workspace/.cache/godot-data`) are gitignored and are **not** always present. When missing, only the export gates fail — `L2_linux_export_smoke` and `L2_windows_cross_export` in `bash tools/run_ci_checks.sh` (all other L0–L2/L4 gates pass). Fix without booting the MCP stack: re-run `install_cloud_dev.sh`'s template step, or download `Godot_v4.7-stable_export_templates.tpz` from the Godot 4.7-stable release and copy its `templates/*` into that folder. The install guard checks for a `version` file but the tpz ships `version.txt`, so a stale `install_cloud_dev.sh` may re-download every run — guard on `version.txt` instead.

> **Gotcha — GUI run in the cloud VM has no GPU.** Running the game or editor on `DISPLAY=:1` logs Vulkan/audio init failures and auto-falls back to OpenGL3 + the dummy audio driver; the Forward+ game window may render grey and close quickly. This is a driver limitation, **not** a boot/data regression. Verify boot health via the **headless** path instead: `godot4 --headless --path game --quit-after 5` (exit 0 = clean; a missing required `game/data` JSON prints `Missing data file: …` and exits 1), the headless unit runner `bash tools/run_unit_tests.sh`, and the exported binary `build/TidesOfUrashima.x86_64 --headless --audio-driver Dummy --quit-after 5`.

> **Note — lint/test/build/run of the game do NOT need the commercial MCP stack.** `bash tools/run_docs_ci_checks.sh` and `bash tools/run_ci_checks.sh` (which cover gdlint/ruff/mypy/shellcheck lint, headless Godot unit tests, and Linux/Windows export smoke) pass with no GDAI/Godot MCP Pro zips and no API keys — this mirrors GitHub `game-ci.yml`. The MCP stack (`ensure_mcp_stack.sh`, the committed `.cursor/environment.json` `start`) is only for AI scene-authoring in the live editor and hard-fails without the commercial plugins; skip it for pure lint/test/build/run.

# Tides of Urashima — Emotional Pacing Chart

**Version:** 1.0 (Pre-build)  
**Cross-refs:** `docs/STORYBOARD.md`, `docs/GDD.md` §9

---

## Beat chart (main path ~150 min)

| Time | Scene | Emotion | Combat | Notes |
|------|-------|---------|--------|-------|
| 0:00 | SC-00 | Mystery, duty | — | Prologue — kappa-turtle |
| 0:02 | SC-01 | Disorientation | — | Movement teach |
| 0:08 | SC-02 | Dread, emptiness | — | Environmental storytelling |
| 0:12 | SC-03 | Guilt, accusation | — | Yuzu voice |
| 0:18 | SC-04 | Urgency, warmth | — | Roku anchor |
| 0:22 | SC-05 | Tension → agency | Tutorial | First fight |
| 0:28 | SC-06 | Wonder, unease | Optional | Dungeon enter |
| 0:35 | SC-07 | Quiet focus | — | Puzzle breather |
| 0:42 | SC-08 | Horror, guilt | Medium | Wraith pair |
| 0:50 | SC-09 | Confrontation | **Boss** | Solo peak |
| 0:58 | SC-10 | Melancholy resolve | — | Yuzu join |
| 1:05 | SC-11 | Seduction, wrongness | — | Flashback |
| 1:12 | SC-12 | Awe | Medium | Party complete; Tide Wraith ×2 gate approach (`enc_sc12_palace_wraiths`) |
| 1:22 | SC-13 | Revelation, weight | — | Box truth |
| 1:32 | SC-14 | Discipline, epic | Miniboss | Yuzu shines |
| 1:45 | SC-15 | Catharsis | **Final boss** | Phases |
| 1:55 | SC-16 | Stillness, agency | — | Choice |
| 2:00 | SC-17* | Ending tone | — | Credits |

**Combat density:** Front-loaded Act I–II; Act III boss-heavy. Puzzle at ~35 min prevents fatigue.

---

## Intensity curve

```
High │              ★ Wraith        ★★ Sentinel  ★★★ Keeper
     │         ★ Crab    ★★ Wraiths
     │    explore──puzzle──┘              choice
Low  │ prologue──village dread──────────────────ending
     └────────────────────────────────────────────────► time
       0        30        60        90       120    150 min
```

---

## Design rules from chart

1. **No combat** before SC-05 (~22 min) — establish mood through exploration and dialogue
2. **Breather** before boss at SC-07 puzzle
3. **Emotional low** after SC-09 win → Yuzu join heals tone
4. **Choice** immediately after peak combat — no extra grind

#!/usr/bin/env bash
# Enforce GodotPrompter + GDAI MCP role rules (.cursorrules §0, docs/ops/agents/MCP_STACK.md).
...
if [[ "$FAIL" -gt 0 ]]; then
  ...
  echo "Docs: .cursorrules §0, docs/ops/agents/MCP_STACK.md, docs/ops/agents/GDAI_CLOUD_SETUP.md"
  exit 1
fi

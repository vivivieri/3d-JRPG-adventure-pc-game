...
Per-track generation prompts for **Tides of Urashima** BGM. Source of truth: `game/data/audio/ace_step_prompts.json`. QA targets (loudness, duration, jury scope): `game/data/audio/audio_qa_catalog.json`. Hero emotional briefs: `docs/briefs/audio/`.
...
4. Export WAV → convert to `.ogg` per `docs/design/audio/AUDIO_PRODUCTION_GUIDE.md`
5. `python3 tools/check_audio_technical.py` + `review_audio_vision.py` (hero tracks) per `docs/design/audio/AUDIO_QA.md`
6. `python3 tools/register_asset.py` + `docs/design/art/LICENSES.md` (MIT / ACE-Step)

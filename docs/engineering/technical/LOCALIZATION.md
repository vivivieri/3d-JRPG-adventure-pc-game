...
> **Ship data:** `game/data/**` JSON and `game/locale/translations.csv` include `en` / `ja` / `zh` / `zh-Hant` inline text. **All locales** get VO files in Phase 7: 12 clips × (`en` + `ja` + `zh`) + 12 × 2 zh-Hant dialects = **60 OGG files** (`docs/design/vision/VO_HIT_LIST.md`).

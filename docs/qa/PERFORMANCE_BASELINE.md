...
**Policy:** `L3_perf_review` PASS requires evidence on a **ship baseline** — **`reference_linux_cloud`** (cloud snapshot / Linux depot) and **`reference_pc_gtx1060`** (Windows depot before M6 prod). See `docs/qa/PLATFORM_SUPPORT.md`.
...
**Snapshot required:** `build` metadata must show env-build snapshot — not JIT. `docs/agents/CLOUD_SNAPSHOT_LAUNCH.md`.
...
|---------|-------|--------|
| Godot | **4.7** stable | `docs/technical/TECH_STACK.md` |
| Renderer | **Forward+** | `project.godot` |
| **Graphics quality** | **Medium** | `docs/art/RENDERING_GUIDE.md` §10 |
...
See `docs/cheat-sheets/RR_CHEATSHEET.md` §Performance review — scene/shader/material/mesh/light/fog changes and post-fix regression.
...
When fixing perf-related bugs, re-run §7.2 on the affected zone **plus** `docs/qa/QA_AND_BUG_PROCESS.md` §6 (adjacent scenes, `INT-*` if flows changed).
...
| Doc | Contents |
|-----|----------|
| `docs/qa/PLATFORM_SUPPORT.md` | **Linux + Windows ship policy; cloud dev parity** |
| `docs/art/RENDERING_GUIDE.md` §10 | Low / Medium / High presets |
| `docs/world/ENVIRONMENT_KITS.md` §9 | LOD + material batching |
| `docs/cheat-sheets/RR_CHEATSHEET.md` | Performance review workflow |
| `docs/qa/QA_AND_BUG_PROCESS.md` §6 | Post-fix regression |
| `steam/STORE_PAGE.md` | Public system requirements |
| `docs/ci-cd/STEAM_RELEASE_CHECKLIST.md` | M6 hardware smoke |

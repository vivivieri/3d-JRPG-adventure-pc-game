...
Use **`--minimal`** mode in Cursor (35 tools) so MCP Pro does not overlap GDAI for scene editing. See `docs/agents/MCP_STACK.md`.
...
```bash
# GDAI — see docs/agents/GDAI_CLOUD_SETUP.md (commercial zip)
...
| GDAI + MCP Pro both edit scenes | **Rule:** GDAI builds; MCP Pro tests only (`docs/agents/MCP_STACK.md`) |
...
- `docs/agents/MCP_STACK.md` — which tool to use when
- `docs/technical/PLUGIN_COMPATIBILITY.md` — Godot 4.7 verification matrix
- `docs/agents/GDAI_CLOUD_SETUP.md` — GDAI install + cloud snapshot

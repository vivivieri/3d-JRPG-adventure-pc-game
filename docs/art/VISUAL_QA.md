...
**Cross-refs:** `docs/art/ART_AUTOMATION_PIPELINE.md` §5, `docs/art/MODEL_QA.md` (asset QA before in-scene), `docs/qa/QA_REMEDIATION_LOOP.md` (FAIL → fix loop), `docs/qa/ACCEPTANCE_CRITERIA.md` (gate `L2_visual_*`)
...
Per `docs/qa/AI_TESTING_SPEC.md` §5.2 step 7:
...
**On FAIL:** `python3 tools/qa_remediation_brief.py --jury <path>.jury.json --log-attempt` — see `docs/qa/QA_REMEDIATION_LOOP.md`.
...
Subjective feel, pacing, audio, localization — `docs/qa/PLAYTEST_SCRIPT.md`. Runs **after** L0–L5 on release candidate only.
...
**Rule:** Configure **≥2** vision APIs. **Pass only if ≥2 return `acceptance.valid_pass: true`** (all V1–V6 + confidence ≥ 0.65). See `docs/qa/ACCEPTANCE_CRITERIA.md`.

...
authority: qa
tokens_est: 1984
summary: "An agent can wire a scene “done” while the **main story soft-locks**, a **quest never advances**, or **combat hangs** — then patch the same trigger logic repeat"
---
...
**Rule:** Flow tasks pass **data validation (L0) → integration scenarios (L4) → E2E spine (L5)** before human playtest. Every FAIL produces a **flow remediation brief** with one changed lever — same as art/audio (`docs/ops/qa/QA_REMEDIATION_LOOP.md`).

**Cross-refs:** `docs/ops/qa/AI_TESTING_SPEC.md`, `docs/engineering/technical/TECHNICAL_DESIGN.md`, `docs/design/world/QUEST_AND_FLAGS.md`, `docs/ops/qa/QA_REMEDIATION_LOOP.md`, `docs/ops/qa/ACCEPTANCE_CRITERIA.md`, `game/data/qa/remediation_playbook.json` §`flow` / §`data`

...
**Cross-refs:** `docs/qa/FLOW_QA.md`, `docs/vision/PACING_CHART.md`, `docs/gameplay/GAME_FEEL.md`, `docs/gameplay/COMBAT_SYSTEMS.md`, `docs/vision/ENDING_DESIGN.md`, `docs/qa/ACCEPTANCE_CRITERIA.md`, `docs/qa/QA_REMEDIATION_LOOP.md`
...
Godot logger (autoload)  ──>  user://playtest/<run_id>.jsonl
                     ...
                     tools/analyze_playtest_telemetry.py
                                        │
     report (pacing / combat / progression / endings)  ──>  docs/qa/QA_REMEDIATION_LOOP.md
```
...
`FAIL` = completion/ending/no-combat-before-gate (hard rules); other flags are `WARN` (tuning signals). `WARN`/`FAIL` → open a remediation item per `docs/qa/QA_REMEDIATION_LOOP.md`.
...
```gdscript
extends Node
## Local-only JSONL playtest telemetry. No PII. See docs/qa/PLAYTEST_TELEMETRY.md.

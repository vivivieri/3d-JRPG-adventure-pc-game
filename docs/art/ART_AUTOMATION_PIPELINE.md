...
**Cross-refs:** `docs/art/ART_DIRECTION.md`, `docs/agents/MCP_STACK.md`, `docs/art/RENDERING_GUIDE.md`, `docs/audio/AUDIO_PRODUCTION_GUIDE.md`, `docs/art/ASSET_COMPLIANCE.md`, `docs/art/GENERATION_READINESS.md` (per-asset briefs + human-expectation gaps), `.cursorrules` §0
...
```
1. READ  docs/art/ART_DIRECTION.md §1 palette for zone
...
- Kenney Nature: **dev greybox only** — not ship art (`docs/art/LICENSES.md`).
...
```
1. READ  docs/art/CHARACTER_BIBLE.md poly budgets + docs/art/GENERATION_READINESS.md row (write docs/generation_briefs/<id>.md if missing)
...
12. `docs/art/MODEL_QA.md` — GLB lint + turntable jury before import; `docs/art/VISUAL_QA.md` after in-scene
13. `docs/qa/ACCEPTANCE_CRITERIA.md` — cite gate id + measured values in agent report; FAIL → `QA_REMEDIATION_LOOP.md`
```
...
Maps dominant hues toward zone rows in `docs/art/ART_DIRECTION.md` §1. Agents run this **after every external gen**, before `register_asset.py`.
...
| Selective VO | ElevenLabs via `generate_ai_vo.sh` | 12 lines only — `docs/vision/VO_HIT_LIST.md` |
| SFX layers | Freesound **CC0-only** + procedural | Register each file |

**No human mix pass or commission** on the ship path. Quality gate = `docs/audio/AUDIO_QA.md` (BGM technical + hero jury A6/A7; P0 VO technical + jury V6/V7) + L6 listen.

**On any art/audio/model QA FAIL:** `docs/qa/QA_REMEDIATION_LOOP.md` — brief + one lever change before rebuild (max 3 attempts).

---

## 8. Quality gates (M5)

Before marking M5 complete (`docs/workflow/MILESTONES.md`). **All gates must meet `docs/qa/ACCEPTANCE_CRITERIA.md`** — WARN/SKIP is not ship PASS.
...
- [ ] No FMV in `game/` — cinematics are Godot-only (`docs/ui/CINEMATICS.md`)
...
- `docs/agents/MCP_STACK.md` — MCP R&R map (tiered requirements)
- `docs/art/ART_DIRECTION.md` — palette, silhouettes, poly budgets

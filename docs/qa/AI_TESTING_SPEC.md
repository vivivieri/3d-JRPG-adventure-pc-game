...
Human QA (`docs/qa/PLAYTEST_SCRIPT.md`) is **Phase 8 / ship gate only**, and **always after** the full AI automated suite.
...
| **L6** | `docs/qa/PLAYTEST_SCRIPT.md` | After L5 on RC | **Ship** |
...
| 7 | Capture viewport **screenshot** | Save to `artifacts/screenshots/`; vision jury per `docs/art/VISUAL_QA.md` §2G |
...
**On scenario FAIL:** `bash tools/qa_emit_remediation.sh flow-scenario INT-<ID>` — see `docs/qa/FLOW_QA.md`.
...
See `docs/qa/PLAYTEST_SCRIPT.md` for step-by-step script.

### 8.4 Bug flow

Human findings → `docs/qa/QA_AND_BUG_PROCESS.md` → AI agent fixes → **re-run L0–L5** → human retest affected paths only.
...
| Create/edit zone scene | **GDAI only** (`docs/agents/MCP_STACK.md`) |
| JSON flag after quest stage | **L1 unit test** |

See `docs/agents/MCP_STACK.md` for conflict rules and install.
...
| `docs/qa/PLAYTEST_SCRIPT.md` | L6 human script |
| `docs/workflow/AI_DEV_WORKFLOW.md` | Build policy + acceptance criteria |
| `docs/qa/ACCEPTANCE_CRITERIA.md` | Measurable QA gates |

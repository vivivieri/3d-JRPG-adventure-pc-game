...
**Cross-refs:** `docs/design/audio/AUDIO_PRODUCTION_GUIDE.md`, `docs/design/audio/AUDIO_DIRECTION.md`

---

## Intent (one sentence)

SC-17c Drift ending — open tragedy, boat rows away, palace glimmers below.

## Emotional intent (jury + human rubric)

| Field | Value |
|-------|-------|
| Primary mood | Tragic refusal |
| Secondary mood | Endless sea loneliness |
| Audience read | Men 20–30 — melancholy coastal JRPG; restrained NieR-like |
| Listen read (automated jury) | Sparse shakuhachi, slow waves, film outro |
| In-game feel (human L6 only) | Loop seam with dialogue duck; zone crossfade taste |
| Must avoid | Heroic resolve, upbeat adventure, vocals |
| Story anchor | SC-17c Drift |
...
```bash
bash tools/generate_ai_bgm.sh --track cine_ending_drift_hero
python3 tools/check_audio_technical.py --track cine_ending_drift_hero
python3 tools/review_audio_vision.py --track cine_ending_drift_hero --min-pass 2
```

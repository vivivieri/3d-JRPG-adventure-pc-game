...
check "QA policy docs" test -f docs/ops/qa/ACCEPTANCE_CRITERIA.md
check "setup script executable" test -x tools/setup_dev_environment.sh
check "implementation plan doc" test -f docs/ops/workflow/IMPLEMENTATION_PLAN.md
check "R&R compliance (no hand-built scenes)" bash tools/check_rr_compliance.sh
check "rendering guide" test -f docs/design/art/RENDERING_GUIDE.md

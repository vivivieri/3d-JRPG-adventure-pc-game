...
| Stop for human | **L6 UAT** after L0–L5 on RC (`docs/qa/PLAYTEST_SCRIPT.md`) |
...
See `docs/agents/GDAI_CLOUD_SETUP.md` for plugin + panel **Start**.
...
Follow docs/agents/PM_AGENT_RUNBOOK.md exactly.
...
| Prompt | Post Slack/email/checklist link to `docs/qa/PLAYTEST_SCRIPT.md`; do not run game code |
...
**Scheduled monitoring (GitHub):** `.github/workflows/factory-watchdog.yml` runs every 2h, calls `run_factory_watchdog.sh --recover` **only when unhealthy**. This is stall insurance — not primary PM dispatch. See `docs/agents/FACTORY_WATCHDOG.md`.

**PM webhook also handles `watchdog_recovery`** — same Automation A; add watchdog branch to PM prompt (see `docs/agents/FACTORY_WATCHDOG.md` §5).

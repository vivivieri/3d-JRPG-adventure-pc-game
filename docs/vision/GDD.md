...
| Skills (total) | 14 player (unique IDs; `strike` shared) + 6 enemy — see `docs/gameplay/SKILLS_BIBLE.md` |
| Playtime | 2–3 hours |
...
- **Scope note:** Reverse-gravity rooms cut from v1; see `docs/world/ENVIRONMENT_KITS.md`
...
- Bosses require reading patterns (telegraphed "intent" UI) — see `docs/gameplay/BOSS_DESIGNS.md`
- No grinding required for story completion on Normal — see `docs/gameplay/ENCOUNTER_TABLE.md`
...
| Shops | Roku's cache — potions, antidote, skill scroll (one-time); see `docs/gameplay/ENCOUNTER_TABLE.md` §7 |
...
**Build order:** `docs/workflow/IMPLEMENTATION_PLAN.md` Phases 0–8 are authoritative. **M5 = art rebuild** (Phase 7) before **M6 = Steam ship** (Phase 8).
...
- See `docs/technical/LOCALIZATION.md` for translator workflow
...
| Asset inconsistency | Single style bible + vertical slice gate (`docs/art/ART_DIRECTION.md` §10) |
| License violations | `docs/art/ASSET_COMPLIANCE.md` + `tools/check_asset_compliance.sh` before ship |

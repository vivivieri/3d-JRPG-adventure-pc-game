...
the schema (cross-referenced to docs/ops/qa/ACCEPTANCE_CRITERIA.md gates). This is a
development QA aid — a red metric opens a remediation item
(docs/ops/qa/QA_REMEDIATION_LOOP.md), it does not by itself block ship.
...
def print_report(agg: dict[str, Any], checks: list[dict]) -> None:
    ...
    if n_fail or n_warn:
        print("On WARN/FAIL: open a remediation item (docs/ops/qa/QA_REMEDIATION_LOOP.md) — change one lever, re-measure.")
...
def build_markdown_report(agg: dict[str, Any], checks: list[dict], chart_paths: list[Path]) -> str:
    ...
    if chart_paths:
        ...
        for p in chart_paths:
            lines.append(f"- `{p}`")
    lines += ["", "_Dev-time Games User Research telemetry — feeds docs/ops/qa/QA_REMEDIATION_LOOP.md. Not a ship gate._"]
    return "\n".join(lines) + "\n"

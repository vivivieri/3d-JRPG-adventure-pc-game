...
static func _add_ground_cover(root: Node3D, zone_id: String, palette: Dictionary) -> void:
	...
	match zone_id:
		...
		"ruined_village":
			...
			_add_village_worn_paths(cover, palette)
		...
		"tidal_caves":
			...
			_add_cave_path_lane(cover, palette, zone_id)
			_add_cave_chamber_floor(cover, palette, zone_id)
			_scatter_cave_flora(cover, Vector3(0, 0, -8), 6.0, 21.0, 3 if _screenshot_mode() else 9)
...
static func _add_palace_marble_bands(parent: Node3D, palette: Dictionary) -> void:
	...
	for z in [-26.0, -14.0, -2.0, 10.0]:
		...
		var plane := PlaneMesh.new()
		plane.size = Vector2(20.0, 1.35)
		plane.orientation = PlaneMesh.FACE_Y
		band.mesh = plane
		band.position = Vector3(0, 0.08, z)
		band.material_override = band_mat
		parent.add_child(band)


static func _add_palace_court_aisle(parent: Node3D, palette: Dictionary) -> void:
...
static func _add_village_mud_patches(parent: Node3D, palette: Dictionary) -> void:
	for patch in [Vector3(-3, 0.02, 5), Vector3(6, 0.02, -4), Vector3(-8, 0.02, -2), Vector3(2, 0.02, -8)]:
...
static func _add_village_worn_paths(parent: Node3D, palette: Dictionary) -> void:
	var path_mat := StandardMaterial3D.new()
	path_mat.albedo_color = Color("#8A7A62")
	path_mat.roughness = 0.96
	_apply_zone_texture(path_mat, "ruined_village", "ground", "ground", "ground")
	for segment in [
		{"center": Vector3(-0.5, 0.04, 4.5), "size": Vector2(2.2, 12.0), "rot": -18.0},
		{"center": Vector3(1.5, 0.04, -1.0), "size": Vector2(1.8, 10.0), "rot": 8.0},
		{"center": Vector3(-5.0, 0.04, 4.0), "size": Vector2(1.6, 7.5), "rot": 72.0},
		{"center": Vector3(6.5, 0.04, 0.5), "size": Vector2(1.5, 6.0), "rot": -12.0},
	]:
		var strip := MeshInstance3D.new()
		strip.name = "VillagePath"
		var plane := PlaneMesh.new()
		plane.size = segment["size"]
		plane.orientation = PlaneMesh.FACE_Y
		strip.mesh = plane
		strip.position = segment["center"]
		strip.rotation_degrees.y = segment["rot"]
...
static func _add_cave_wet_patches(parent: Node3D, palette: Dictionary, zone_id: String) -> void:
	for spot in [Vector3(-4.6, 0.03, -18), Vector3(3.2, 0.03, -9.5), Vector3(-2.0, 0.03, -4.0)]:
		var puddle := MeshInstance3D.new()
		puddle.mesh = TerrainShapes.flood_pool_mesh(0.9 if spot.z < -10 else 0.65, 0.55 if spot.z < -10 else 0.45, 12)
		puddle.position = spot
		WaterMaterial.apply_to_mesh(puddle, palette, zone_id, true)
		parent.add_child(puddle)


static func _add_cave_path_lane(parent: Node3D, palette: Dictionary, zone_id: String) -> void:
	var lane := MeshInstance3D.new()
	lane.name = "CavePathLane"
	lane.mesh = TerrainShapes.cave_path_lane_mesh(-30.0, 14.0, 2.8)
	lane.position = Vector3(0, -0.17, -7.5)
	var mat := StandardMaterial3D.new()
	mat.albedo_color = Color("#6E6860")
	mat.roughness = 0.78
	mat.emission_enabled = true
	mat.emission = Color("#4A4540") * 0.12
	_apply_zone_texture(mat, zone_id, "ground", "ground", "ground")
...
static func _add_cave_chamber_floor(parent: Node3D, palette: Dictionary, zone_id: String) -> void:
	...
	chamber_light.name = "ChamberFill"
	chamber_light.light_color = palette.get("accent", Color.CYAN)
	chamber_light.light_energy = 0.35
	chamber_light.omni_range = 14.0
	chamber_light.position = Vector3(2.0, 2.4, -8.0)
	parent.add_child(chamber_light)
	var rim_spots: Array = [
		Vector3(-9.5, 0, -2.0),
		Vector3(11.0, 0, -3.5),
		Vector3(-8.0, 0, -13.5),
	...
	if _screenshot_mode():
		rim_spots = [Vector3(-9.5, 0, -2.0), Vector3(11.0, 0, -3.5), Vector3(2.0, 0, -18.0)]
	for spot in rim_spots:
...
static func _scatter_cave_flora(
	...
	var scale_min := 0.75 if _screenshot_mode() else 1.05
	var scale_max := 0.95 if _screenshot_mode() else 1.45
	_scatter_flora_kinds(
		parent,
		center,
		radius_x,
...
static func _apply_environment(root: Node3D, palette: Dictionary, zone_id: String) -> void:
	...
	match zone_id:
		...
		"dragon_palace_gate":
			...
			env.glow_bloom = 0.18
			env.ambient_light_color = palette.get("light", Color.WHITE) * 0.55
		_:
			env.fog_density = 0.012
...
static func _add_village_set(parent: Node3D, palette: Dictionary, zone_id: String) -> void:
	...
	var lantern := JapaneseNatureMeshes.build_stone_lantern(
		palette.get("structure", Color("#5C4A3A")).lightened(0.1),
		palette.get("light", Color("#F0E0C8")),
	)
	lantern.position = Vector3(-4.5, 0, -3.5)
	parent.add_child(lantern)
	var pine := JapaneseNatureMeshes.build_japanese_pine(
		palette.get("structure", Color("#5C4A3A")),
		palette.get("moss", Color("#3D5C4A")),
		6.0,
	)
	pine.position = Vector3(-12, 0, 6)
	parent.add_child(pine)
	var pine2 := JapaneseNatureMeshes.build_japanese_pine(
		palette.get("structure", Color("#5C4A3A")).darkened(0.05),
		palette.get("moss", Color("#4A6A48")),
		5.2,
	)
	pine2.position = Vector3(11, 0, -8)
	parent.add_child(pine2)
	var bamboo_grove := JapaneseNatureMeshes.build_bamboo_cluster(7, Color("#4A7A42"), Color("#6A5A38"))
	bamboo_grove.position = Vector3(-14, 0, -2)
	parent.add_child(bamboo_grove)
...
static func _add_flood_chamber_wetness(parent: Node3D, pos: Vector3, palette: Dictionary, zone_id: String) -> void:
	...
	for offset in [Vector3(-1.2, 0.02, 0.8), Vector3(0.8, 0.02, -0.6), Vector3(0, 0.02, 1.2)]:
		...
		drip.mesh = TerrainShapes.flood_pool_mesh(0.55, 0.42, 10)
...
static func _add_torii(parent: Node3D, pos: Vector3, palette: Dictionary, zone_id: String) -> void:
	var torii := JapaneseNatureMeshes.build_torii(
		palette.get("accent", Color("#8B3A2A")),
		palette.get("structure", Color("#5C4A3A")),
	)
	torii.name = "ToriiProp"
	torii.position = pos
	parent.add_child(torii)
	PropLibrary.spawn("bush", torii, Vector3(0, 0, 0.9), 0.0, 0.9, false)
	PropLibrary.spawn("grass", torii, Vector3(-1.2, 0, 0.6), 20.0, 0.85, false)
	PropLibrary.spawn("grass", torii, Vector3(1.1, 0, 0.5), -15.0, 0.8, false)
...
static func _add_pier(parent: Node3D, pos: Vector3, palette: Dictionary, zone_id: String) -> void:
	...
	PropLibrary.spawn("bush", pier, Vector3(5.2, 0, 0.6), 0.0, 0.85, true)
	PropLibrary.spawn("rock_small_a", pier, Vector3(4.8, 0, -0.4), 15.0, 0.75, true)
...
static func _add_beach_set(parent: Node3D, palette: Dictionary, zone_id: String) -> void:
	var box := JapaneseNatureMeshes.build_lacquer_box(
		Color("#6B1818"),
		Color("#C9A030"),
	)
	box.position = Vector3(1.2, 0, 8.2)
	parent.add_child(box)
	var drift_a := JapaneseNatureMeshes.build_driftwood_log(2.8, palette.get("structure", Color("#5C4A3A")))
	drift_a.position = Vector3(-3.5, 0, 7.5)
	drift_a.rotation_degrees = Vector3(5, 32, 8)
	parent.add_child(drift_a)
	var drift_b := JapaneseNatureMeshes.build_driftwood_log(2.1, palette.get("structure", Color("#5C4A3A")).darkened(0.1))
	drift_b.position = Vector3(4.2, 0, 5.8)
	drift_b.rotation_degrees = Vector3(-3, -18, 5)
	parent.add_child(drift_b)
	var rock := JapaneseNatureMeshes.build_moss_rock(
		0.55,
		palette.get("ground", Color.GRAY).darkened(0.15),
		palette.get("moss", Color("#3D5C4A")) if palette.has("moss") else Color("#3D5C4A"),
	)
	rock.position = Vector3(-5.5, 0, 2.5)
	parent.add_child(rock)
	var seaweed := JapaneseNatureMeshes.build_seaweed_cluster(palette.get("water", Color("#1A5A6A")))
	seaweed.position = Vector3(-1.5, 0, -2.5)
	parent.add_child(seaweed)
	var bamboo := JapaneseNatureMeshes.build_bamboo_cluster(5, Color("#4A7A42"), Color("#6A5A38"))
	bamboo.position = Vector3(-9, 0, 10)
	parent.add_child(bamboo)
	for offset in [Vector3(-4, 0, 6), Vector3(3, 0, 4), Vector3(5, 0, 7)]:
		PropLibrary.spawn("rock_small_a", parent, offset, randf_range(0, 360), randf_range(0.9, 1.15), false)
		PropLibrary.spawn("grass", parent, offset + Vector3(0.5, 0, 0.4), randf_range(0, 360), 0.85, false)
